...
def evaluation_form_view(request, proponent_id, project_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not evaluation.overall_rating:
                evaluation.overall_rating = 0

            evaluation.save()
            print(f"Overall Rating: {evaluation.overall_rating}")
            return render(request, 'thank_you.html', {'stored_overall_rating': evaluation.overall_rating})
            # return redirect('evaluation_thank_you', stored_overall_rating=evaluation.overall_rating)  # Redirect to a thank-you page after submission
    else:
        form = EvaluationForm()
