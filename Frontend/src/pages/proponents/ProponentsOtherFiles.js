    ...
    const handleSubmit = async () => {
        ...
        if (attachedFiles.length > 0) {
            attachedFiles.forEach((file) => {
                formData.append("other_files", file); // Match the backend field
            });
        } else {
                                            ...
                                            <tr key={submission.submission_id} className="border-b hover:bg-gray-100">
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                                    ...
                                                    >
                                                        {submission.file_name
                                                            ? submission.file_name.replace("other_files/", "").length > 20
                                                                ? `${submission.file_name.replace("other_files/", "").slice(0, 17)}...`
                                                                : submission.file_name.replace("other_files/", "")
                                                            : "No File"}
                                                    </a>
                                                </td>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 text-center">
                                                    {submission.status === "Approved" ? (
                                                        ...
                                                        <button
                                                            onClick={() => handleDelete(submission.submission_id, "other_files")} // Use the correct model name
...
export default ProponentsOtherFiles;
