...
const EvaluationForm = ({ token }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            ...formData,
            activityRatings: JSON.stringify(formData.activityRatings),
            speakerRatings: JSON.stringify(formData.speakerRatings),
        };

        try {
            const response = await axios.post(
                `/api/monitoring/evaluation/fill/${token}/`, 
                payload
            );
            setSuccess(true);
            setLoading(false);
        ...
        <div className="flex items-center justify-center min-h-screen bg-cover bg-fixed" style={{ backgroundImage: `url("${process.env.PUBLIC_URL}/images/bg-login3.png")` }}>
            <div className="overflow-auto h-screen p-8 bg-white rounded-lg shadow-lg max-w-2xl">
                <form onSubmit={handleSubmit}>
                    ...
                    {success && <p className="text-green-500 text-center mt-4">Evaluation submitted successfully!</p>}
                </form>
            </div>
        </div>
    );
